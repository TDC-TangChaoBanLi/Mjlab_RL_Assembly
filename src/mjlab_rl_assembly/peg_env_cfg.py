...
class ReachTargetCommand(CommandTerm):
    ...
    def _update_command(self) -> None: # 继承
        ...
        if site_names[0] == _PEG_SITE_NAME:
            ...
            vec = torch.tensor([0.0, 0.0, self.cfg.z_offset], device=self.device).expand(self.num_envs, -1)
            target_pos = peg_site_pos + quat_apply(peg_site_quat, vec)
...
def _infer_robot_joint_names_from_mj_model(mj_model, robot_prefix: str):
    """
    从 MuJoCo model 中推断 UR5e 的 hinge/slide 关节名。
    会跳过 free joint，只保留真正的机械臂关节。
    """
    joint_names = []
    ...
    if len(joint_names) == 0:
        raise ValueError(
            ...
            "Please check joint names in your MJCF."
        )
...
def _get_joint_qpos_indices(mj_model, joint_names):
    """根据 MuJoCo joint name 得到对应 qpos 地址。"""
    qpos_indices = []
    ...
    for joint_name in joint_names:
        ...
        if jtype not in [
            ...
            raise ValueError(
                ...
                "This helper only supports 1-DoF arm joints."
            )
...
def _build_full_qpos_for_mink(
    ...
    注意：
    - mink.Configuration 需要完整 qpos，长度必须等于 mj_model.nq。
    - 这里以 model.qpos0 为底，然后把 UR5e 关节角写进去。
    - 对固定基座 UR5e，这是最稳的方式。
    """
    ...
    if len(robot_joint_pos_np) != len(robot_joint_qpos_indices):
        raise ValueError(
            ...
            f"but inferred joint qpos indices length = {len(robot_joint_qpos_indices)}. "
            "Please check robot joint names and joint order."
        )
...
def reset_peg_pose_and_ur5e_ik(
    ...
    修正点：
    1. peg 目标从世界坐标转为 env-local 坐标；
    2. 使用 mink.Configuration.integrate_inplace，而不是 qpos + dq * dt；
    3. mink 使用完整 qpos，写回仿真时只写 UR5e 关节角；
    4. 自动根据 MuJoCo joint name 找 UR5e 关节 qpos 地址。
    """
    ...
    # 防止上下界写反，例如 y: (-0.3, -0.7)
    low = torch.minimum(ranges[:, 0], ranges[:, 1])
    high = torch.maximum(ranges[:, 0], ranges[:, 1])
    ...
    peg_pos_w = pose_samples[:, 0:3] + env.scene.env_origins[env_ids]

    peg_quat_w = quat_from_euler_xyz(
        pose_samples[:, 3],
        pose_samples[:, 4],
        pose_samples[:, 5],
    )

    peg_root_pose = torch.cat([peg_pos_w, peg_quat_w], dim=-1)
    peg_entity.write_mocap_pose_to_sim(peg_root_pose, env_ids=env_ids)

    # ----------------------------------------------------
    # 2. 先把机器人恢复到默认关节状态
    # ----------------------------------------------------
    # if robot_entity.is_articulated:
    default_joint_pos = robot_entity.data.default_joint_pos[env_ids].clone()
    default_joint_vel = robot_entity.data.default_joint_vel[env_ids].clone()
    ...
    # ----------------------------------------------------
    # 3. 读取 peg site 位姿，构造末端目标位姿
    # ----------------------------------------------------
    site_ids, site_names = peg_entity.find_sites(_PEG_SITE_NAME)

    if len(site_ids) == 0 or site_names[0] != _PEG_SITE_NAME:
        raise ValueError(
            f"Peg site '{_PEG_SITE_NAME}' not found on entity '{asset_cfg.name}'"
        )

    peg_site_id = site_ids[0]

    peg_site_pos_w = peg_entity.data.site_pos_w[env_ids, peg_site_id]
    peg_site_quat_w = peg_entity.data.site_quat_w[env_ids, peg_site_id]

    offset_vec = torch.tensor(
        [0.0, 0.0, z_offset],
        device=env.device,
        dtype=peg_site_pos_w.dtype,
    ).unsqueeze(0).expand(len(env_ids), -1)

    target_pos_w = peg_site_pos_w + quat_apply(peg_site_quat_w, offset_vec)
    target_quat_w = peg_site_quat_w

    # 关键修正：
    # peg_site_pos_w 是世界坐标，mink 单模型 IK 通常应使用局部坐标。
    target_pos_local = target_pos_w - env.scene.env_origins[env_ids]
    target_quat_local = target_quat_w

    # ----------------------------------------------------
    # 4. 找 EE site 名称和 UR5e 关节 qpos 地址
    # ----------------------------------------------------
    ...
    # 5. 逐个环境求 IK
    # ----------------------------------------------------
    solved_joint_pos_list = []

    for local_index, env_id_int in enumerate(env_ids.tolist()):

        # 当前环境默认关节角，作为 IK 初值
        q_joint_seed = default_joint_pos[local_index].detach().cpu().numpy().copy()

        if q_joint_seed.shape[0] != num_robot_joints:
            raise ValueError(
                f"default_joint_pos has {q_joint_seed.shape[0]} joints, "
                f"but inferred MuJoCo UR5e joints are {num_robot_joints}: "
                f"{robot_joint_names}. "
                "This means the entity joint order and MuJoCo joint inference do not match."
            )
        ...
        # mink.SE3 的顺序是 [qw, qx, qy, qz, x, y, z]
        target_wxyz_xyz = np.concatenate(
            [target_quat_np, target_pos_np],
            axis=0,
        )
        ...
        #                     "UR5e/COLLISION_ur_wrist_2_link_1",
        #                     "UR5e/COLLISION_ur_wrist_3_link_2",
        #                     "UR5e/VISUAL_lens_link_0",
        #                 ],
        #                 ["peg/COLISIONS_peg_base", "table/COLISIONS_main_body_peg_f2370d47"],
        #             ),
        #         ],
        #         gain=0.85,
        ...
        for _ in range(ik_iterations):
            try:
                ...
                print(
                    f"[IK warning] env_id={env_id_int}, "
                    f"mink.solve_ik failed: {exc}"
                )
                break
        ...
        if not ik_success:
            ...
            print(
                f"[IK warning] env_id={env_id_int}, "
                f"IK not fully converged. "
                f"pos_err={pos_err:.6f}, ori_err={ori_err:.6f}"
            )

        qpos_solution_full = configuration.q.copy()

        # 只取 UR5e 关节角，不能把完整 qpos 写入 write_joint_position_to_sim
        q_joint_solution = qpos_solution_full[robot_joint_qpos_indices].astype(np.float32)

        solved_joint_pos_list.append(q_joint_solution)

    # ----------------------------------------------------
    # 6. 批量写回 UR5e 关节状态
    # ----------------------------------------------------
    solved_joint_pos = torch.tensor(
        np.stack(solved_joint_pos_list, axis=0),
        device=env.device,
        dtype=torch.float32,
    )
